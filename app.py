...
def verify_email():
    ...
    if request.method == "POST":
        ...
        return redirect(url_for("start"))
    return render_template("verify.html", email=current_user.email)
...
def profile():
    ...
    average_score = sum(result.score for result in user_results) / len(user_results) if user_results else 0
...
@login_required
def admin_page():
    allowed_emails = {"ddavidok845@gmail.com", "oborozhniios@vybor.zp.ua"}
